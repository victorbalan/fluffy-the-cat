function Level(level, stage, dimensions){
	...
	function checkAtFinish(player) {
		var finishBounds = finishPos.getBounds();
		if(player.x <= finishBounds.x + finishBounds.width &&
			player.x >= finishBounds.x &&
			player.y <= finishBounds.y + finishBounds.height &&
			player.y >= finishBounds.y){
				return true;
		}
		return false;
	}
	...
	return {
		...
		startPos: function(){ return startPos; }
	}
}
